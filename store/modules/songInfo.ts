...
function findMatchingAyaSong(
  ...
  let ayaSong = categories.flatMap((category) => category.entries).find((ayaSong) => {
  ...
  reducers: {
    ...
    initSongInfo: (state: SongInfo, action: PayloadAction<InitValues>) => {
      const { udonGroups, ayaCats, defaultSortBy, time, udonFiles, udonUrls } =
        action.payload;
      const genericGroups = udonGroups.map(udonGroup => ({
          ...
          entries:
            udonGroup.songInfos?.map(udonSong => {
              ...
              const originalUrlFromUdon = udonUrls.find((url) => url.id === udonSong.id)?.url;
              const originalUrl = originalUrlFromUdon ? [originalUrlFromUdon] : ayaSong?.originalUrl;
              const checksum = udonFiles.find((f) => f.id === udonSong.id)?.md5 ?? ayaSong?.checksum;
              return {
      ...
      // 全部
      const allSongEntries = genericGroups.flatMap((groups) => groups.entries).reduce((acc: GenericVideo[], entry) => {
...
export const selectSongInfo = (state: RootState) => ({
  ...
  songTime: state.SongInfo.updatedAt,
